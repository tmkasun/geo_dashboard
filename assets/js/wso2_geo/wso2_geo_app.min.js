...
function focusOnSpatialObject(a){var b=currentSpatialObjects[a];if(!b)return $.UIkit.notify({message:"Spatial Object <span style='color:red'>"+a+"</span> not in the Map!!",status:"warning",timeout:3E3,pos:"top-center"}),!1;clearFocus();selectedSpatialObject=a;map.setView(b.marker.getLatLng(),17,{animate:!0});$("#objectInfo").find("#objectInfoId").html(selectedSpatialObject);b.marker.openPopup();toggled||($("#objectInfo").animate({width:"toggle"},100),toggled=!0);getAlertsHistory(a);b.drawPath();setTimeout(function(){createChart();
chart.load({columns:[b.speedHistory]})},100)}function clearFocus(){selectedSpatialObject&&(spatialObject=currentSpatialObjects[selectedSpatialObject],spatialObject.removePath(),spatialObject.marker.closePopup(),selectedSpatialObject=null)};
