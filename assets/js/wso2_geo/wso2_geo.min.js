var ApplicationOptions={colors:{states:{NORMAL:"blue",WARNING:"orange",OFFLINE:"grey",ALERTED:"red",UNKNOWN:"#19FFFF"},application:{header:"grey"}},constance:{SPEED_HISTORY_COUNT:20,NOTIFY_INFO_TIMEOUT:1E3,NOTIFY_SUCCESS_TIMEOUT:1E3,NOTIFY_WARNING_TIMEOUT:3E3,NOTIFY_DANGER_TIMEOUT:5E3},messages:{app:{}},leaflet:{iconUrls:{normalIcon:"assets/img/markers/arrow_normal.png",alertedIcon:"assets/img/markers/arrow_alerted.png",offlineIcon:"assets/img/markers/arrow_offline.png",warningIcon:"assets/img/markers/arrow_warning.png",
defaultIcon:"assets/img/markers/default_icons/marker-icon.png"}}};var debugObject,showPathFlag=!1,currentSpatialObjects={},selectedSpatialObject,webSocketURL="ws://localhost:9764/outputwebsocket/DefaultWebsocketOutputAdaptor/geoDataEndPoint",websocket,waitTime=1E3;
...
"</span> Info : "+a.properties.information);var b=this.createLineStringFeature(this.state,this.information,[this.latitude,this.longitude]);this.pathGeoJsons.push(b);if(selectedSpatialObject==this.id){var c=new L.polyline(b.geometry.coordinates,this.getSectionStyles(a.properties.state));c.bindPopup("Alert Information: "+b.properties.information);b=this.path[this.path.length-1].getLatLngs();b=new L.polyline([b[b.length-1],[this.latitude,this.longitude]],this.getSectionStyles());this.path.push(b);this.path.push(c);
b.addTo(map);c.addTo(map)}}this.marker.setLatLng([this.latitude,this.longitude]);this.marker.setIconAngle(this.heading);this.marker.setIcon(this.stateIcon());0<this.pathGeoJsons.length?this.pathGeoJsons[this.pathGeoJsons.length-1].geometry.coordinates.push([a.geometry.coordinates[1],a.geometry.coordinates[0]]):(b=this.createLineStringFeature(this.state,this.information,[a.geometry.coordinates[1],a.geometry.coordinates[0]]),this.pathGeoJsons.push(b));selectedSpatialObject==this.id&&(this.updatePath([a.geometry.coordinates[1],
a.geometry.coordinates[0]]),chart.load({columns:[this.speedHistory.getArray()]}),map.setView([this.latitude,this.longitude]));this.popupTemplate.find("#objectId").html(this.id);this.popupTemplate.find("#information").html(this.information);this.popupTemplate.find("#speed").html(this.speed);this.popupTemplate.find("#heading").html(this.heading);this.marker.setPopupContent(this.popupTemplate.html())};
...
function setWithinAlert(a){var b=JSON.stringify(map._layers[a].toGeoJSON().geometry).replace(/"/g,"'"),c=$("#queryName").val(),d=$("#areaName").val();data={parseData:JSON.stringify({geoFenceGeoJSON:b,executionPlanName:createExecutionPlanName(c),areaName:d}),executionPlan:"within",customName:d,queryName:c,cepAction:"deploy"};$.post("controllers/set_alerts.jag",data,function(b){$.UIkit.notify({message:'<span style="color: dodgerblue">'+b.status+"</span><br>"+b.message,status:"success"==b.status?"success":
...
$(e).attr("onClick","showAlertInMap(this)");$(e).attr("data-id",b.id);$(e).attr("data-latitude",b.latitude);$(e).attr("data-longitude",b.longitude);$(e).attr("data-state",b.state);$(e).attr("data-information",b.information);c.append(e)})})}function createExecutionPlanName(a){return"geo_within"+(a?"_"+a:"")+"_alert"}function closeAll(){$(".modal").modal("hide");setTimeout(function(){$.UIkit.offcanvas.hide()},100)};var drawControl;
